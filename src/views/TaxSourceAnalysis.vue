    ...
    <el-card class="mt16" v-if="analyzing">
      ...
      <el-timeline>
        <el-timeline-item
          ...
          :key="step.title"
          :type="i < currentStep ? 'success' : i === currentStep ? 'primary' : 'info'"
          :hollow="i > currentStep"
        >
          <strong>{{ step.title }}</strong>
          <p class="step-desc">{{ step.desc }}</p>
          <el-progress v-if="i === currentStep" :percentage="progress" :stroke-width="6" class="step-progress" />
        </el-timeline-item>
      </el-timeline>
    </el-card>
    ...
    <el-card class="mt16" v-if="showResult">
      <template #header>
        <div class="card-header">
          <span>分析结果</span>
          <el-button type="success" @click="genReport"><el-icon><Document /></el-icon> 生成报告</el-button>
        </div>
      </template>
      ...
      <el-card class="mt16 abnormal-card" shadow="never">
        <template #header>
          <span class="section-title danger-text"><el-icon><Warning /></el-icon> 异常指标清单</span>
        </template>
        <el-table :data="filteredAbnormalList" stripe size="small">
      ...
      <el-card class="mt16 analysis-card" shadow="never">
        <template #header>
          <span class="section-title primary-text"><el-icon><Document /></el-icon> AI风险原因分析</span>
        </template>
...
const industries = [...industryOptions, '住宿餐饮业']
...
const filteredAbnormalList = computed(() => {
  const nameKeyword = query.value.name.trim()

  return abnormalList.filter((item) => {
    const matchesName = !nameKeyword || item.company.includes(nameKeyword)
    const matchesIndustry = !query.value.industry || item.indicator.includes(query.value.industry) || item.company.includes(query.value.industry)
...
.step-desc { color:#6b7280; margin:4px 0 0; }
...
.abnormal-card { border:1px solid #fecaca; background:#fff5f5; }
.analysis-card { border:1px solid #e0e7ff; background:#eef2ff; }
.section-title { display:inline-flex; align-items:center; gap:6px; }
.danger-text { color:#dc2626; }
.primary-text { color:#1a56db; }
</style>
