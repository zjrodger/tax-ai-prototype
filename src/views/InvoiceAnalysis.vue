    ...
    <el-card class="mt16">
      <template #header>
        <div class="card-header">
          ...
          <el-button type="primary" @click="aiDialog=true"><el-icon><MagicStick /></el-icon> AI智能分析</el-button>
        </div>
      </template>
      <el-table :data="tableData" stripe highlight-current-row @row-click="showDetail">
        ...
        <el-table-column prop="deviation" label="偏差率" width="90" align="right">
          <template #default="{row}"><span :style="{color: Math.abs(row.deviation)>1?'#dc2626':'#059669', fontWeight:'bold'}">{{ row.deviation>0?'+':'' }}{{ row.deviation }}%</span></template>
        </el-table-column>
      ...
      <div class="mt16" style="text-align:right">
        <el-pagination background layout="total, prev, pager, next" :total="86" :page-size="10" />
      </div>
    </el-card>

    <!-- AI分析弹窗 -->
    <el-dialog v-model="aiDialog" title="AI 智能分析" width="600px" top="5vh">
      <div class="ai-process">
        <div v-for="(step,i) in aiSteps" :key="i" class="ai-step" :class="{active:i<=activeStep, done:i<activeStep}">
      ...
      <div v-if="activeStep>=aiSteps.length" class="ai-result">
        ...
        <p style="margin-top:12px;color:#374151">共检测到 <strong style="color:#dc2626">3家</strong> 企业税负率异常偏低，建议重点关注：平顶山永安矿业有限公司（偏差-2.3%）、南阳恒通制造（偏差-1.8%）、周口鑫源贸易（偏差-1.5%）。完整报告已生成。</p>
      </div>
      <template #footer>
        <el-button @click="aiDialog=false">关闭</el-button>
        <el-button type="primary" :loading="activeStep<aiSteps.length" @click="runAI" :disabled="activeStep>=aiSteps.length">开始分析</el-button>
      </template>
    </el-dialog>
...
<script setup>
import { ref } from 'vue'
const industries = ['制造业','批发零售业','建筑业','采矿业','交通运输业','房地产业','信息技术服务业']
const query = ref({ period:'2026-02', name:'', industry:'' })
const tableData = ref([
  ...
  { company:'安阳盛达房地产有限公司', industry:'房地产业', invoiceCount:145, invoiceAmount:125000, taxRate:9.8, avgRate:9.5, deviation:0.3, status:'正常' },
])
const handleSearch = () => {}
const handleReset = () => { query.value = { period:'2026-02', name:'', industry:'' } }
const aiDialog = ref(false)
const detailDialog = ref(false)
const selectedRow = ref(null)
const showDetail = (row) => { selectedRow.value = row; detailDialog.value = true }
...
const runAI = () => {
  ...
  const timer = setInterval(() => {
    activeStep.value++
    if (activeStep.value >= aiSteps.length) clearInterval(timer)
  }, 1200)
}
</script>
